...
class modJoomCatHelper extends JoomInterface
{
  ...
  function getCats(&$dberror)
  {
    ...
    switch($this->getConfig('catmode'))
    {
      // top rated
      case 1:
        $query .= ', IF(SUM(jg.imgvotes) > 0, SUM(' . JoomHelper::getSQLRatingClause('jg'). ') / count(jg.id), 0) AS catrating, SUM(jg.imgvotes) AS catimgvotes';
        break;
      // most viewed
      case 2:
        $query .= ', SUM(jg.hits) AS cathits';
        break;
      // default, last added
      default:
        break;
    ...
    switch($this->getConfig('catmode'))
    {
      // top rated
      case 1:
           $query .= ' GROUP BY ca.cid ORDER BY catrating DESC';
        break;
      // most viewed
      case 2:
           $query .= ' GROUP BY ca.cid ORDER BY cathits DESC';
        break;
      // default, last added
      default:
    ...
    $query .= ' LIMIT 0,' . $this->getConfig('categorycount');
  ...
  function prepareSlideshow(&$imgelement, $slideshowclass, $slideshowid, $catid, $catname, $img_rows)
  {
    ...
    if($firstcall)
    {
      ...
      $doc->addScript(JURI::base() . 'modules/mod_joomcat/assets/mod_joomcat.js');
